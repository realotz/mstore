...
func (c *volumeServiceClient) SaveFile(ctx context.Context, in *SaveFileReq, opts ...grpc.CallOption) (*v1.Empty, error) {
	out := new(v1.Empty)
	err := c.cc.Invoke(ctx, "/api.service.storage.v1.volume.VolumeService/SaveFile", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (UnimplementedVolumeServiceServer) SaveFile(context.Context, *SaveFileReq) (*v1.Empty, error) {
...
func _VolumeService_SaveFile_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(SaveFileReq)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(VolumeServiceServer).SaveFile(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/api.service.storage.v1.volume.VolumeService/SaveFile",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(VolumeServiceServer).SaveFile(ctx, req.(*SaveFileReq))
	}
	return interceptor(ctx, in, info, handler)
}
