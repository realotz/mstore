...
func RegisterVolumeServiceHTTPServer(s *http.Server, srv VolumeServiceHTTPServer) {
	...
	r.PUT("/api/v1/volume/{id}/files", _VolumeService_SaveFile0_HTTP_Handler(srv))
...
func _VolumeService_SaveFile0_HTTP_Handler(srv VolumeServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in SaveFileReq
		if err := ctx.Bind(&in); err != nil {
			return err
		...
		http.SetOperation(ctx, "/api.service.storage.v1.volume.VolumeService/SaveFile")
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.SaveFile(ctx, req.(*SaveFileReq))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
func (c *VolumeServiceHTTPClientImpl) SaveFile(ctx context.Context, in *SaveFileReq, opts ...http.CallOption) (*v1.Empty, error) {
	...
	pattern := "/api/v1/volume/{id}/files"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation("/api.service.storage.v1.volume.VolumeService/SaveFile"))
	opts = append(opts, http.PathTemplate(pattern))
